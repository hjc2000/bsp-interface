#include "StandardEthernetPort.h"
#include <bsp-interface/di/console.h>
#include <bsp-interface/di/delayer.h>

void bsp::StandardEthernetPort::SoftwareResetPHY()
{
...
bool bsp::StandardEthernetPort::SupportAutoNegotiation()
{
	/* BSR bit3 指示是否支持自动协商。
	 *		0：不支持自动协商。
	 *		1：支持自动协商。
...
void bsp::StandardEthernetPort::EnableAutoNegotiation()
{
...
bool bsp::StandardEthernetPort::AutoNegotiationCompleted()
{
	/* BSR bit5 指示自动协商是否完成。
	 *		0：自动协商未完成。
	 *		1：自动协商完成。
...
void bsp::StandardEthernetPort::EnablePowerDownMode()
{
	uint32_t bcr = ReadPHYRegister(0);
	bcr |= 0x0800U;
	WritePHYRegister(0, bcr);
...
void bsp::StandardEthernetPort::DisablePowerDownMode()
{
	uint32_t bcr = ReadPHYRegister(0);
	bcr &= ~0x0800U;
	WritePHYRegister(0, bcr);
...
void bsp::StandardEthernetPort::EnableLoopbackMode()
{
...
void bsp::StandardEthernetPort::DisableLoopbackMode()
{
	uint32_t register_value = ReadPHYRegister(0);
	register_value &= ~0x4000U;
	WritePHYRegister(0, register_value);
...
bool bsp::StandardEthernetPort::IsLinked()
{
